...
const App: React.FC = () => {
  ...
  return (
    <div 
      className="flex flex-col h-screen max-w-md mx-auto bg-[#fafafa] relative overflow-hidden shadow-2xl"
    ...
    >
      ...
      {view === 'assistants' ? (
        ...
        >
          ...
          <div 
            ref={scrollRef}
            className="flex-1 overflow-y-auto px-2 pt-6 pb-44 no-scrollbar"
          >
          ...
          {/* Input Area */}
          <div className="absolute bottom-0 left-0 right-0 p-[0.25rem_1.25rem_1.25rem_1.25rem] bg-[#fafafa]/90 backdrop-blur-lg z-20">
            {/* Toolbar Area */}
            <div className={`absolute -top-10 left-5 right-5 flex items-center justify-between transition-opacity duration-300 ${isRecording ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
              <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-1">
                <button 
                  onClick={() => setIsDeepThinking(!isDeepThinking)}
                  className={`flex items-center gap-1 px-2.5 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-tight transition-all duration-300 border ${
                    isDeepThinking 
            ...
            {attachments.length > 0 && !isRecording && (
              <div className="flex gap-3 overflow-x-auto no-scrollbar mb-4 px-2 py-1 items-center">
                {attachments.map((att) => (
                  <div key={att.id} className="relative flex-shrink-0 animate-in fade-in zoom-in duration-300">
                    <div className="w-20 h-20 rounded-2xl border border-gray-100 bg-white shadow-sm flex items-center justify-center overflow-hidden">
                      {att.isLoading ? (
                        <div className="flex flex-col items-center justify-center gap-1">
                          <Loader2 size={18} className="text-blue-500 animate-spin" />
                          <span className="text-[9px] text-gray-400 font-bold uppercase tracking-tighter">Uploading</span>
                        </div>
                        ...
                        <div className="flex flex-col items-center justify-center gap-1 p-2">
                          ...
                          <span className="text-[9px] text-gray-500 line-clamp-1 break-all px-1 text-center font-medium">{att.name}</span>
                        </div>
                      )}
                    </div>
                    <button 
                      onClick={() => removeAttachment(att.id)}
                      className="absolute -top-1.5 -right-1.5 bg-white/90 backdrop-blur-sm text-gray-800 rounded-full p-1 shadow-md border border-gray-100 hover:bg-white transition-all active:scale-90"
                    >
            ...
            <div className={`bg-white rounded-[32px] shadow-lg border border-gray-100 flex items-center px-4 py-2 gap-2 transition-all duration-300 min-h-[56px] ${isRecording ? 'scale-[0.98] opacity-50' : ''}`}>
              <input type="file" ref={fileInputRef} onChange={handleFileChange} multiple className="hidden" accept="image/*" />
              <button onClick={() => fileInputRef.current?.click()} className="p-2 text-gray-800 hover:text-black transition-colors">
              ...
              <div className="flex-1 flex justify-center items-center px-1 overflow-hidden">
                {inputMode === 'voice' ? (
                  <button
                    ...
                    className={`w-full py-2.5 rounded-full text-[17px] font-medium text-gray-900 select-none transition-all active:scale-[0.98] leading-normal ${isRecording ? 'bg-gray-100' : 'bg-transparent'}`}
                    style={{ height: '44px' }}
                  >
                    按住 说话
                  </button>
                  ...
                  <textarea
                    ...
                    className="w-full py-2.5 bg-transparent border-none focus:ring-0 text-[17px] text-gray-900 placeholder:text-gray-400 resize-none no-scrollbar outline-none font-light leading-relaxed"
              ...
              <button 
                ...
                className={`p-2 transition-all ${inputMode === 'text' ? 'text-blue-500' : 'text-gray-800'}`}
              >
      ...
      {isRecording && (
        ...
        >
          <div className={`w-full h-[70%] rounded-t-[60px] flex flex-col items-center justify-center transition-all duration-500 relative ${
            ...
            <div className="flex flex-col items-center gap-6 z-10 px-10 text-center max-sm:px-6">
            ...
            <div className="mt-12 h-10 w-0.5 bg-gradient-to-b from-gray-200 to-transparent rounded-full opacity-50"></div>
          </div>
        </div>
      )}
    </div>
