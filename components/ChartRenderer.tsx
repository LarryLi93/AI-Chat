...
const MetricCard: React.FC<{ label: string; value: string | number; trend?: string }> = ({ label, value, trend }) => {
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm flex flex-col gap-1 min-w-[140px] flex-1 text-left">
...
export const ChartRenderer: React.FC<{ json: string }> = ({ json }) => {
  try {
    ...
    // Fix: Explicitly type useState to include 'bar' as Type narrowing in the ternary was excluding it from the inferred state type.
    const [currentType, setCurrentType] = useState<ChartProps['type']>(config.type === 'bar' ? 'horizontalBar' : config.type);
    ...
    const renderChartContent = () => {
      switch (currentType) {
        case 'line':
          return (
            <LineChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Legend verticalAlign="top" align="left" height={36} wrapperStyle={{ fontSize: '11px', fontWeight: 500, paddingLeft: '20px' }} />
          ...
          return (
            <BarChart data={data} margin={{ top: 20, right: 10, left: -20, bottom: 0 }}>
              ...
              <Bar dataKey="value" fill="#3b82f6" radius={[6, 6, 0, 0]} barSize={32}>
                <LabelList dataKey="value" position="top" style={{ fontSize: '10px', fill: '#94a3b8', fontWeight: 600 }} offset={8} />
              </Bar>
            </BarChart>
          );
        case 'horizontalBar':
          return (
            <BarChart layout="vertical" data={data} margin={{ top: 10, right: 40, left: 10, bottom: 0 }}>
              ...
              <Bar dataKey="value" fill="#8b5cf6" radius={[0, 6, 6, 0]} barSize={24}>
                <LabelList dataKey="value" position="right" style={{ fontSize: '10px', fill: '#94a3b8', fontWeight: 600 }} offset={8} />
              </Bar>
            </BarChart>
          );
        case 'pie':
          return (
            <PieChart>
              <Pie data={data} cx="30%" cy="50%" innerRadius={60} outerRadius={90} paddingAngle={8} dataKey="value">
                {data.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} strokeWidth={0} />
                ))}
              </Pie>
              <Tooltip contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.1)', fontSize: '12px' }} />
              <Legend verticalAlign="middle" align="right" layout="vertical" wrapperStyle={{ fontSize: '11px', fontWeight: 500 }} />
            </PieChart>
          );
        case 'scatter':
          return (
        ...
        case 'metric':
          return (
            <div className="flex flex-wrap gap-3 py-1 text-left">
              {data.map((item, idx) => (
    ...
    return (
      <div className="w-full bg-white rounded-[32px] border border-gray-100 p-6 my-4 shadow-[0_8px_40px_rgba(0,0,0,0.03)] animate-in fade-in zoom-in-95 duration-500 text-left">
        {/* Row 1: Title */}
        ...
        {/* Row 2: Type Switcher - HorizontalBar first */}
        ...
        {/* Row 3: Chart Content - Left Aligned */}
        <div className={currentType === 'metric' ? "w-full text-left" : "h-[320px] w-full text-left"}>
          {currentType === 'metric' ? (
            renderChartContent()
          ) : (
    ...
    return (
      <div className="bg-red-50 text-red-500 p-6 rounded-[28px] text-xs font-medium my-4 border border-red-100/50 flex flex-col gap-2 text-left">
